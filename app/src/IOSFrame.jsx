...
export function IOSKeyboard({ dark = false }) {
  ...
  const mkKey = (content, { w, flex, ret, fs = 25, k } = {}) => (
    <div key={k} style={{
  ...
  const row = (keys, pad = 0) => (
    <div style={{ display: 'flex', gap: 6.5, justifyContent: 'center', padding: `0 ${pad}px` }}>
      {keys.map(l => mkKey(l, { flex: true, k: l }))}
    </div>
  );
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 14.25, alignItems: 'center' }}>
          {mkKey(icons.shift, { w: 45, k: 'shift' })}
          <div style={{ display: 'flex', gap: 6.5, flex: 1 }}>
            {['z','x','c','v','b','n','m'].map(l => mkKey(l, { flex: true, k: l }))}
          </div>
          {mkKey(icons.del, { w: 45, k: 'del' })}
        </div>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          {mkKey('ABC', { w: 92.25, fs: 18, k: 'abc' })}
          {mkKey('', { flex: true, k: 'space' })}
          {mkKey(icons.ret, { w: 92.25, ret: true, k: 'ret' })}
        </div>
      </div>
