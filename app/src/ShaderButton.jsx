...
export const ShaderContext = React.createContext(SHADER_DEFAULTS);

export function ShaderProvider({ value, children }) {
  return <ShaderContext.Provider value={value}>{children}</ShaderContext.Provider>;
}

export function ShaderButton({ onClick, children, className = 'fb-btn', label = '+ Add task' }) {
  ...
  const haloStyle = (enabled && tweaks.glow) ? {
    boxShadow: '0 0 0 1.5px var(--ink), 0 8px 24px rgba(255, 207, 3, 0.55)',
  } : null;
