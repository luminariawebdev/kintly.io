...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  const apply = React.useCallback(() => {
    ...
    if (lastPostedScale.current !== scale) {
      ...
      try { window.parent.postMessage({ type: '__dc_zoom', scale }, '*'); } catch (_) {}
    }
  ...
  React.useEffect(() => {
    ...
    const onHostMsg = (e) => {
      ...
      if (d && d.type === '__dc_set_zoom' && typeof d.scale === 'number') {
        ...
        try { window.parent.postMessage({ type: '__dc_present' }, '*'); } catch (_) {}
  ...
  const cloneStyled = (src) => {
    ...
    if (src.nodeType === 1) {
      ...
      if (src.tagName === 'CANVAS') try {
        const im = document.createElement('img'); im.src = src.toDataURL(); im.setAttribute('style', txt); return im;
      } catch {}
    }
