// screens-main.jsx — Main /app one-page screen
// Sticky header + Anchor tabs + 3 sections (Tasks / Calendar / Notes)
...
function TasksSection({ onAdd }) {
  ...
  return (
    <section className="fb-sec" id="sec-tasks" data-screen-label="Tasks">
      ...
      <div style={{ marginTop: 10 }}>
        {groups.length === 0 &&
        <div className="kbd-hint" style={{ padding: '20px 0' }}>
...
function CalendarSection({ onAdd }) {
  ...
  // upcoming list: next 4 events from today
  const upcoming = EVENTS.filter((e) => e.day >= today).slice(0, 4);

  return (
    <section className="fb-sec" id="sec-calendar" data-screen-label="Calendar">
      <div className="fb-sec-hd">
        <div>
...
function NotesSection({ onAdd }) {
  return (
    <section className="fb-sec" id="sec-notes" data-screen-label="Notes">
      <div className="fb-sec-hd">
        <div>
    ...
    <Modal
      ...
      <div className="field">
        ...
        <input
          ...
          placeholder="Any context, links, etc." />
      ...
      <div className="field">
        ...
        <div className="assignee-picker">
          ...
          <button
            key={u.id}
            className={'pick' + (assignee === u.id ? ' on' : '')}
            onClick={() => setAss(u.id)}>
          ...
          <button
            className={'pick unassign' + (assignee === null ? ' on' : '')}
            onClick={() => setAss(null)}>
            Unassigned</button>
        </div>
      </div>
...
function AddEventModal({ open, onClose }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={<>Add <em>event</em></>}
      footer={
      <button className="fb-btn solid" onClick={onClose}>Save event</button>
      }>
...
function AddNoteModal({ open, onClose }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={<>Pin a <em>note</em></>}
      footer={
      <button className="fb-btn solid" onClick={onClose}>Post note</button>
      }>
...
// ─────────────────────────────────────────────────────────────
// MainApp — the full one-page /app screen
function MainApp({ initialTab = 'tasks', initialModal = null, initialScroll = null }) {
  const [tab, setTab] = React.useState(initialTab);
  const [modal, setModal] = React.useState(initialModal);
  const scrollRef = React.useRef(null);
  const tabsRef = React.useRef(null);

  // Scroll to section when tab is tapped, accounting for sticky header+tabs height
  ...
  // Watch scroll position to highlight the current tab
  React.useEffect(() => {
    const wrap = scrollRef.current;
    if (!wrap) return;
    let raf;
  ...
  // Initial scroll to a section after mount, if requested
...
Object.assign(window, {
  INITIAL_TASKS, EVENTS, NOTES,
  TasksSection, CalendarSection, NotesSection,
  AddTaskModal, AddEventModal, AddNoteModal,
  MainApp
});